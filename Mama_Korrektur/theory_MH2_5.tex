...
The description of fluids is based on three physical principles. \fxunsure{Kommen die drei Prinzipien jetzt sofort?, oder bezieht sich das auch auf den Text hinter Eq. 2.1?}
The first one  states that a fluid can be treated as a continuum \citep{schlichting_boundary-layer_2017} \fxchange{Hört sich an, wie ne alte Erkenntnis, dann gehört hier als erstes eine alte Originalquelle hin}.
...
as\fximprove{Müssen die Variablen nicht eingeführt werden?, mind in einem Nomenklaturverzeochnis: $\rho: xxx$, gilt für die nächste Gleichung ebenfalls }
...
Building on this,\fxunsure{However??}  the behaviour of fluids is often split into three groups with different characteristic properties:
...
\fximprove{ein Halbsatz zu transitional}
...
The flow behaviour in the immediate vicinity of the wall is in all these\fxunsure{Hier wurden mE keine Fälle aufgezählt: also besser "most cases"?} cases very similar due to the universal nature of small scales turbulences \citep{pope_turbulent_2015}XX umformulieren.
...
\fxbmark{nach Mail 1 - 12.07. um 11:30 }
...
The velocity sufficiently distant from the wall, known as the \benennung{free-stream velocity} $U_0$, is usually much higher than zero. This observation indicates the presence of   a transition region where an acceleration of the fluid occurs (XXQUELLE).
...
For simplicity,  free-stream velocity $U_0$  is often set to be 1. \fximprove{diesen satz vlt eher bei non-dim? XX - hat Velocity nicht ne }
The streamwise velocity component inside the boundary layer is denoted  $u(x,y)$.
Often, the thickness of the boundary layer is modelled using three different parameters \fxchange{Quelle einfügen}: 
...
This approach is adopted from(XXQUELLE)  since it facilitates a  more precise measurement in comparison to the boundary-layer thickness. 
%To describe the boundary-layer with the Reynolds number, 
The boundary-layer is related to the Reynolds number, 
\fxunsure{Vorsicht Hier habe ich inhaltlich  geändert, weil ich ihn nicht verstehe}
with different Reynolds numbers being defined based on the introduced displacement measures:
different Reynolds numbers based on the introduced displacement measures are defined as
...
Applying it on the \acrshort{nse} leads to the \acrfull{rans} \fxchange{Quelle einfügen}. 
...
In order to undertake a study of the phenomena present in a boundary layer, it is first necessary to define the so-called viscous scales, since the ranges of dimensions and velocity variations are much smaller than in the free stream (see Pope, 2015, Turbulent Flow).
%
...
With these quantities, the original length and velocity scales can be non-dimensionalised in wall units as shown in \citep{moin_fundamentals_2024}:
\begin{equation}
    y^+ = \frac{y}{\delta_v} = \frac{y u_\tau}{\nu}, \qquad
    u^+ = \frac{U}{u_\tau}.
\end{equation}

The merits of these non-dimensional scales are Twofold.  Firstly, they are universal for all wall-bounded flows enabling the comparison between flows of different physical dimensions. XX unbedingt QUELLE XX
Moreover, the non-dimensionalisation facilitytes a detailed investigation of the velocity profile inside the \acrshort{tbl}.
Prandtl (XXQUELLE) observed that the \acrshort{tbl} contains an inner part where the mean velocity profiles are independent of the free-stream quantities $U_0$ and $\delta_{99}$ such that the mean velocity profile can be fully described by the plus\fxchange{was ist das?} units.
...
For turbulent flows, the Reynolds stresses are significant measures\fxchange{characteristics?}, since the transport of mean momentum is based on them \citep{moin_fundamentals_2024}. 
...
The diagonal entries therefore\fxinfo{Therefore: Bezug?} contain the normal stresses $\langle u' u'\rangle, \langle v' v'\rangle, \langle w' w'\rangle$.
...
%
In order to resolve this issue, a number of different approaches can be adopted  to derive additional equations such ensuring that all terms can be computed.  These apporaches are described in detail in fundamental books like \citet{wilcox_turbulence_2006,pope_turbulent_2015} XXnoch eines.
The transport equations for the Reynolds stresses can be  determined by XXX \citep{mansour_reynolds-stress_1988}
...
\fxbmark{ende Mail2- 12.07 -14:15}

\subsection{Clauser parameter in turbulent boundary layers}

In boundary layers the free-stream (XX stimmt das?) pressure gradient plays a crucial role since it has a significant impact on the flow behaviour significantly (XXQUELLE).
As demonstrated in \autoref{eq:NSEy}, the pressure for a \acrshort{tbl} is constant along a flat wall  in the wall-normal direction.
...
In contrast, an increase of the velocity \fxunsure{Wie kann die Geschwindigkeit steigen, und das im Kontrast  zu Beschleunigung?} yields a positive or \benennung{\acrfull{apg}}. 
It can thus be concluded that a \acrshort{tbl} with a constant free-stream velocity will have no pressure gradient.
This configuration, designated as \benennung{\acrfull{zpg}}, has been the subject of extensive research due to its status as the natural configuration in a plane \acrshort{tbl}  (XXQUELLE).
...
MThe majority of research conducted ont \acrshort{tbl} has focused on  the utilisation of\acrshort{tbl}  in contexts of straight walls. (XXQUELLE)
However, in many real-world scenarios, the wall may not be completely straight, but rather exhibit curved segments (XXQUELLE). 
The effects in \acrshort{tbl} along curved walls are still poorly studied (XXQUELLE), since the problem involves a range of inherent complications. 
...
As \citet{so_experiment_1973} observed in their experiments without pressure adjustment, the pressure decreases slowly near the beginning of the curvature and then decreases fast once the curvature starts\fxunsure{ist dem Experten klar, dass near the beginning of curvature und "when it starts" sich auf unterschiedliche Bereiche bezieht?}. 
Since these pressure variations have a significant impact on the turbulent structures, an indepth study of curvature effects necessitates the  compensation of these pressure variations (XXQUELLE). 
...
In experimental studies, the achievement of a defined pressure distribution is facilitated by adjusting the position of the upper wall
 \citep{so_experimental_nodate, johnston_experimental_nodate, gillis_turbulent_1983,schwarz_convex_1996}. \fxinform{Soll hier wirklich Zitationen ohne Jahreszahl rein?}
...
First, a boundary layer along a convex wall with \acr  \acrshort{zpg} shows a smaller growth rate compared to a straight developing \acrfull{tbl} \citep{so_experimental_nodate}, \citep{meroney_turbulent_1975}. 
This phenomenon can be attributed to a pressure gradient in \fxunsure{the/a} centripetal direction. 
It balances the centrifugal force of the fluid elements. The application of  centrifugal force  will result  in   a displacement of fluid particles with high velocities  to a new outer position. 
...
In addition, they found that the law of the wall coincides for the curved wall with the same parameters as for flat wall measurements\fxunsure{den Satz verstehe ich nicht}. 
...
%
In a similar manner to  the experimental investigations, a modification of the setup is necessary to compensate for the streamwise pressure gradient
 \citet{appelbaum_systematic_2025}, XX. 
...
With the flat plate as reference case it is, therefore, posible to isolate the effects of curvature.
...
A similar approach was adopted by \citet{bobke_history_2017}  in their study on the effect of history on \acrshort{apg} \acrshort{tbl}\fxunsure{Stimmt der satz so?}.
They copied  the Clauser parameter \fxchange{prüfen!} $\beta(x)$ distribution from a wing \acrshort{dns} on a flat \acrshort{tbl} through the imposition of corresponding velocity profiles along the top wall.
They studied various distributions and showed that even with matching $\beta(x)$ and $Re_{\tau}$ the geometry of the flow\fxunsure{ die Geometrie der wand? } alters the flow \citep{bobke_history_2017}.
...
However, these studies mainly focus on the effect of pressure on the flow characteristics referreing to the argument that the pressure-driven effects superpose the curvature effects \citep{patel_longitudinal_1997}.
...
The experimental results of the \acrshort{rans} demonstrated thtat, in comparison with \acrshort{dns} results, the accuracy of the  turbulence models tested was inadequate in cases involving large curvature \citep{yang_assessment_2016}.
In the study of \citep{appelbaum_systematic_2025} a concave \acrshort{tbl} was investigated. 
In oder to achieve a \acrshort{zpg} distribution the pressure profile from the top wall of a plane \acrshort{tbl} was imposed on the top wall of the curved domain.
...
The objective of this project is to implement and evaluate a noval workflow for the configuration of a curved \acrshort{zpg} \acrshort{tbl}.
