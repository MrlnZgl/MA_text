...
Only the profiles from the optimised simulations are considered here, since the differences between the optimised and the base configuration are negligible in both cases negligible.
...
Up to the log-law region, the profiles  match the reference profile and the linear law perfectly, respectively.\fxchange{habe das log-law hier gelöscht, korrekt?}
...
The velocity decreases in the outer layer of case R100deg90 can be induced by the \acrshort{bc} at the upper wall.
...
Consequently, the velocity gradient between the free stream and the upper wall becomes sufficiently large to influence a substantial portion of the free stream. \fxchange{Alternative: As a result, the velocity gradient imposed between the free stream and the upper wall extends across a significant fraction of the free-stream region, increasing the influence of the upper boundary condition on the flow}
...
The streamwise development of the boundary layer thickness along the domain is recorded in \autoref{fig:R500deg18R100deg90_delta99}.
...
Compared with R100deg90,  case R500deg18 exhibits  a larger \acrshort{tbl} growth rate. This difference is consistent with the stronger stabilising effect of the more intense convex curvature in R100deg90, which suppresses boundary-layer growth \citep{gillis_turbulent_1983,so_experiment_1973}.\fxchange{stimmt das so?}
...
A similar behaviour was already detected for case R100deg30. It  
has also been reported for a concave curved  \acrshortpl{tbl}   by \citet{appelbaum_systematic_2025}.
...
\fxinfo{das verstärkt jetzt die Aussage des letzen Satzes, ok so?}
...
All configurations exhibit a peak near the inflow, results from the tripping  \fxunsure{XXstimttdas??, was ist das?}.
Then, the 
friction coefficient decreases monotonically in all simulations.

For the case R500deg18,  a slight deviation from this decreasing trend occurs at  approximately $s/\delta_0 \approx 75$, corresponding to the onset of the curved section. In the optimised configuration,  this deviation is absent and the decreasing trend continues smoothly, resulting in a lower friction coefficient than in the base configuration. 
...
In constrast, no comparable difference between the base and optimised configurations is observed for case R100deg90. 
Both configurations exhibit a similar deviation at the beginning of  the curved segment and and subsequently follow nearly identical trends until the end of the curvature.
...
Similar to case R100de30 discussed in \autoref{subsec:R100deg30}, both configurations  exhibit distinct peaks at the entrance and exit of the curved  segment. However, these peaks are less pronounced than those observed for R100deg30. 
Within the curved segment, the base configuration shows a smooth trend \fxinfo{for $\beta$?}, whereas  the optimised\fxinfo{??} configuration exhibits small local fluctuations.
...
The optimised configuration reduces the global trend of $\beta$\fxinfo{??} towards zero. 
...
The slope of the pressure  decreases more rapidly along  the curved section, which is consistent with the stronger wall curvature \fxunsure{XXQUELLE}.
...
In contrast, the optimised configuration of R100deg90 still exhibits an increasing free-stream velocity, with an even steeper overall trend than the corresponding base configuration. In addition, pronounced local fluctuations are present, consistent with the behaviour observed in the distributions of $\beta$ and wall pressure.




The analysis of the pressure-related quantities for the cases R500deg18 and R100deg90 reveals substantial differences in the capablities\fxinfo{performance?} of the optimisation algorithm. 
...
Consequently,  the free-stream velocity is balanced on a nearly constant level.
 Although local fluctuations are still present in all analysed quantities, they are less pronounced than in case R100deg30. This behaviour is consistent with the weaker curvature, which induces a smaller pressure gradient and therefore requires less extensive modifications of the pressure field. In addition, the lower curvature may increase the effective influence of individual \benennung{control points}, leading to a smoother distribution of the Clauser parameter.

In contrast, the optimisation performs considerably less effectively for case R100deg90. While the base configuration exhibits trends similar to those observed in the other cases, the optimised solution is dominated by strong local fluctuations, preventing the establishment of the desired pressure distribution. One possible explanation is that the effective influence of each 
\benennung{control point} decreases with increasing curvature. If this hypothesis is correct, geometries with stronger curvature require a larger number of control points to achieve the same level of pressure control. 
In addition, the stronger curvature provokes a larger pressure gradient demanding more substantial modifications of the pressure field. This effect is particularly pronounced for R100deg90. Although R100deg90 and R100deg30 share the same radius of curvature, the longer curved section of R100deg90 results in a larger cumulative pressure-gradient effect.


Overall, the results indicate that the proposed optimisation strategy is effective for configurations with mild convex curvature, where it successfully drives the pressure distribution towards \acrshort{zpg} conditions using a limited number of \benennung{control points}. For geometries with stronger or more extended curvature, however, further development of the optimisation approach is required to achieve comparable performance.
