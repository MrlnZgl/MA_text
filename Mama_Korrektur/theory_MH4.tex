...
The description of fluids is based on three\fxchange{3 Prinzipien verlangen nach 1., 2. und 3....ich sehe (1), ... wo ist (3)?  textuell unklar} physical principles:
...
\begin{gather}
    \frac{\partial \rho}{\partial t} + \frac{\partial (\rho u_i)}{\partial x_i} = \frac{\partial u_i}{\partial x_i} = 0  \\[4pt]
    \begin{aligned}
        \rho &\colon \text{density} & \quad u_i &\colon \text{velocity component}
    \end{aligned} \notag
\end{gather}
...
%Therefore, it is useful to introduce a set of quantities that characterise this near-wall behaviour.
...
\begin{figure}[H]
 ...
 %   \input{images/Uplus_referenceOnly_txt.pgf}
    \caption{Comparison of velocity profiles in viscous scales for different $Re_\theta$, data taken from \citet{balasubramanian_direct_2023}}
...
First, a boundary layer along a convex wall with  \acrshort{zpg} shows a smaller growth rate compared to a straight developing \acrfull{tbl} \citep{so_experimental_nodate, meroney_turbulent_1975}. 
...
The pressure gradient forces the fluid particle in the opposite direction \citep{so_experimental_nodate, meroney_turbulent_1975}.
...
\fxbmark{14.07. - 10:00 - im vorigen Text wurden nur noch übrig gebliebene Rechtschreib/Grammatikprobleme behoben}
...
\acrfull{dns}  necessitates the solution of the \acrshort{nse} in order to compute  $U(x,t)$.
...
In the context of  small turbulent scales are of interest, the  utilisation  of \acrshort{dns} is inevitable.
Due to the high computational demands, \acrshort{dns}  is typically employed in the study of   low to moderate Reynolds numbers. Its application is primarily restricted to research context \fxchange{Quelle}.

The simulation method of \acrshort{dns} is the principal approach employed in  this project.
Furthermore, we  utilised \acrfull{rans} simulations in order to solve the \acrshort{rans} instead of the full \acrshort{nse}.\fxunsure{Prüfen, ob das so stimmt}
...
In order to circumvent the computation of this tensor, a number of methodologies have been formulated for its modelling \fxchange{Quelle}. 
Since the models have different characteristics and accuracies, the selection of the model will have a substantial  impact on the simulation (XX QUELLE).
It is evident that these simulations show a considerably better computational performance.
In case the precise behaviour of the turbulent scales is irrelevant, and the main interest  is the effect of the turbulence on the flow, \acrshort{rans} simulations provide a reasonable compromise (XXQUELLE).
...
When running simulations with \name{neko}, it is possible to track statistics regarding the  simulated flow.  The recording of the statistics can be configured in various manners and is defined in the so-called case file of the simulation.
 ...
 These statistics are used to extract and calculate certain measures\fxunsure{metrics?} of the flow. In order to accomplish this, it is necessary to undertake a process of post-processing of the recorded statistics. For this purpose,  the \name{python} package \name{pysemtools} is employed. 
