...
This phenomenon has been observed in all technical applications where a fluid is confined by a wall, such as the fluid within a pipe or the airfoil \citep{marusic_wall-bounded_2010}.
These flows are known as \benennung{wall-bounded flows}.
It is imperative to note that the processes occurring in proximity to the wall are of particular significance, as they have the potential to induce substantial
 energy loss in the flow \citep{fan_decomposition_2020}.
They are the result of  friction between the wall and the fluid itself. The factors determining this friction are numerous and include  wall roughness, geometry, and viscosity of the fluid \citep{schlichting_boundary-layer_2017}.
A reduction of these losses is important for \fxinfo{wie geht' der Satz weiter?, dann Absatz?}
...
A substantial body of research has been conducted to obtain understanding of the underlying mechanisms  in \acrfull{tbl} along a plane wall, as referenced in \citep{pargal_adverse-pressure-gradient_2022}.
\citet{schlatter_turbulent_2012} studied the impact of inflow conditions and tripping on the development of \acrshort{tbl} along a flat plate.
A thorough investigation of the structures in the \acrshort{tbl} was performed by \citet{jimenez_near-wall_2013}.
Nevertheless, only a limited number of studies have focused on \acrshort{tbl} along curved surfaces, despite the fact that such geometries\fxinfo{??} are common in real-world applications \citep{spalart_direct_2024}.
Already \citet{muck_effect_1985} demonstrated that the study of curvature effects is  nonetheless important, since in their investigations of convex and concave curvature exhibited fundamental differences in the \acrshort{tbl} development.\fxinfo{prüfen, ob das so Sinn macht}
Addtionally, \citet{chiwanga_effect_1993} yielded noteworthy insights   regarding the differences between turbulent structures along a flat wall and a convex curved wall.
...
This approach requires a non-trivial procedure to determine appropriate \acrfull{bc}. Consequently, only a limited number of researchers have addressed the investigation of pressure-isolated curved \acrshort{tbl}.
\citet{appelbaum_systematic_2025}  sought to distinguish the curvature effects from pressure effects on a concave \acrshort{tbl}. \fxchange{Dieser Satz gehört hier iWie nicht mehr hin, oder es braucht mehr Erläuterung}
...
The idea is to impose \acrlong{bc} constraining the velocity at the upper wall of the curved domain.  The \acrlong{bc}   serve to modify the pressure field towards a \acrlong{zpg}.
It is therefore necessary to determine \acrshort{bc} for the velocity field. 
This is achieved through the utilisation of assistant simulations of lower accuracy. 
...
It is possible to adjust the configuration\fxinfo{shape?} of the upper wall by manipulating the position of the movable control points,
...
The algorithm optimises the shape of the upper wall to obtain the prescribed target value of the \benennung{Clauser parameter}denoted by $\beta$, which is a non-dimensionalised variable for the pressure gradient, along the lower wall.
...
In present simulation enables the effects of the curvature on the \acrshort{tbl} to be studied.
...
Next, the theoretical fundamentals are introduced in \autoref{chap:basics}.
...
Finally, the key findings are summarised and an outlook for future investigations is given.
\fxinfo{das ist knapp ;)}
