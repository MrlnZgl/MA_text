...
Therefore, they observed converse adaptations of the boundary layer growth. This finding was reflected in the \acrshort{tbl}, which showed an increased growth rate at the start of the curvature. 
Furthermore, the authors detected varying slopes for the growth rate depending on the pressure configuration.
These effects can be compared to the behaviour of  curved \acrshort{tbl} under study at the transition at the end of the curvature to the straight outlet.
As  previously outlined, a similar effect can be observed.
This similarity is of particular interest, given that it occurs between two distinct positions of the domain constitution.
As demonstrated by \citet{appelbaum_systematic_2025},  this discrepancy was directly observed  at the first transition of the straight inlet to the curved segment. However, in this study the effect is  only evident at the end of the curvature during\fxinfo{?} the transition to the straight outlet.
...
In the initial section\fxinfo{segment?}, both configurations demonstrate a constant value of $\beta$ at zero, which has been already described in detail in \autoref{sec:planeTBLVerif}.
...
The streamwise direction is once more normalised with the reference boundary layer thickness. The pressure is visualised as a difference between the local pressure at the wall $p(s)$ and the mean pressure at the wall within the curved segment of the domain $\langle p_c \rangle$.
This kind of visualisation facilitates the analysis of the pressure distribution along the streamwise direction. For the purpose of simplification, the term 'pressure' or 'pressure distribution' is employed in the following analysis.
...
Throughout  the entire domain  a pressure decrease of $\Delta(p(s)-\langle p_c \rangle) \approx 0.09$ is visible.
...
These discrete \benennung{control points} are connected  by means of a spline. 
The adjustment of a single \benennung{control point} may result in a modified pressure gradient in the cross-section  at the designated  \benennung{design point}.
Here, the pressure gradient is highlighted, since the pressure gradient is the field quantity that is actually influenced by a shape modification of the domain.
...
This behaviour is enforced if the \benennung{control points} are distributed sparsely along the arc. This is due to the fact that  the cross-sections located between the \benennung{control points} depend only on the spline shape selected by the optimiser to connect them.\fxinfo{den letzten Halbsatz: to connect them, versteh ich nicht so ganz}
...
The setup studied here shows more extreme curvature characteristics  in comparison to  the ones already tested with the \acrshort{bo}-\acrshort{gpr}-optimisation. Consequently, no reference values are available concerning the adequate number of \benennung{control points}.
...
However, in the base configuration, local fluctuations are not observed.
...
To examine this in detail, a  analysis of their positions in relation to the positions of the \benennung{control points} would be necessary.\fxinfo{will be necessary, but it is out of the scope of this thesis.}
