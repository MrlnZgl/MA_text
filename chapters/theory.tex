...
The description of a fluid is based on the continuum hypothesis. 
It states that the motion of molecules in a fluid are continuous (XXQUELLE).
In addition we take the viscous property of fludis into account.
XX Hhier noch wie continuity equation erreicht wird
With that hypothesis we can define the continuity equation which states that ....
For viscous incompressible flows this equation is defined as
...
This so-called \name{Navier-Stokes-equation (NSE)} can be written as 
...
With these equations the motions of a fluid can be described (XXQUELLE).
The Navier stokes equations hold universally for all kind of flows.
However, one can split the behaviour of fluids into three groups with diffrent characteristic properties.
...
\begin{equation}
    u_i = \langle u_i \rangle + u_i' \equiv U_i + u_i' \text{ with } \langle u_i
...
Here the only non-zero entry is $\langle u' v'\rangle$.



XX REYNOLDSSTRESSES PRODUCTION TERMS
