...
Next the results of the case R100deg30 are analysed in terms of the optimisation capability.
The simulations of the cases R100deg90 and R500deg18 are compared to analyse how different curvatures affect the \acrshort{tbl}.


\section{Flat section verification}\label{sec:flatTBLVerif}
The following section is employed as a reliability test of the governed results for the \acrshort{tbl} simulations over a curved wall.
...
To ensure that these fluctuations leave the long-term statistics unaffected, we verified the statistics of the simulations over a curved wall in the straight inlet.
In order to undertake this task, we compare the statistics of case the R100deg30 in the optimised and the base configurations with the ones from the flat \acrshort{tbl} setup and the reference data for flat \acrshort{tbl} simulations from \citet{schlatter_assessment_2010}, which has previously been introduced.
The statistics are taken at $\mathrm{Re}_{\theta} = 700$ in all simulations performed. This corresponds to a position at half of the streamwise domain length.
...
Following this, the results of this comparison are presented for case the R100deg30. 
...
The streamwise velocity profile in viscous scaling is illustrated in \autoref{fig:inflowSecVerifU}.
...
The statistical analysis conducted no significant differences between the flat and the curved-wall \acrshort{tbl} simulations, encompassing both the base and the optimised simulation setup of the R100deg30 case.
...
remains close to zero, with a minor balancing region observed at the beginning of the domain. 
With increase distance from the inflow in streamwise direction, a slight increase in the distance between the flat and the curved-wall setups is evident.
This increase reaches a maximum distance of approximately $0.1$ at the termination of the straight section.
...
\section{Curved-wall turbulent boundary layers}\label{sec:curvedResults}

In the following section the results of the curved-wall \acrshort{tbl} simulations are discussed.
Firstly, the results of the case R100deg30 are presented in isolation to analyse the general optimisation performance for a curved-wall setup.
Then, the optimised configurations of the cases R100deg90 and R500deg18 are compared to study the effect of different geometrical configurations on the optimisation and the development of the \acrshort{tbl}.


\subsection{Optimisation analysis for case R100deg30}\label{subsec:R100deg30}
First, the results of the case R100deg30 are presented. 
...
This position is located within the curvature, with a certain distance to both ends to mitigate interfering effects of the geometry changes.
Then, the development of the boundary layer thickness in the streamwise direction is presented. 
...
In addition, it can be seen that the profiles follow the analytical solutions of the linear law and the log law.
This indicates a correct transformation of the velocity components from the fix $xyz$-coordinate system into the streamwise and wall-normal components. 
The correct implementation can be justified by the velocity profiles since in transformed directions the qualitative shape in wall-normal direction should not change in the curved segment.
...
As the last local statistics we focus on the budgets of the \acrshort{tke} in \autoref{fig:R100deg30Re1200Budget}.
...
This position coincides with the start of the curved section which implies that the geometry change impacts the boundary layer growth.
After a short period in streamwise direction, the profiles recover and increase again. 
However, both quantities show a reduced slope such that their growth is decelerated in the curved segment compared to the straight section.
This phenomenon is known and was already detected by \citet{gillis_turbulent_1983}.
In the curved section the growth rate stays constant until the end of the curvature is reached.
A sudden change of both quantities is remarkable at $s/\delta_0 \approx 110$ which marks the end of the curved section.
Reversely to the start of the curvature, here a sudden increase of the growth rate can be detected in all quantities.
Similar, this region is quite short in streamwise direction until the slope levels at a comparable niveau as in the curved section.
This observation forces the statement that the curvature effects the growth of the boundary layer.
Besides this general behaviour for both configurations, some \benennung{local} differences between them can be detected.
In the straight part the configurations perfectly match each other. 
Right before the start of the curved section, a small negative bump is visible for the optimised configuration.
...
It can be seen that for both measures $\delta_{99}$ and $\theta$ of the optimised configuration develops with a higher growth rate than the base configuration.
Additionally, in the optimised configuration the change in the growth rate due to the end of the curved section is less compared to the base profile.
However, the optimised configuration appears to be affected earlier by the end of the curved section.

A similar observation was made by \citet{appelbaum_systematic_2025} in their results of \acrshort{tbl} over a concave-curved wall.
The authors used a different approach, as we described in \autoref{sec:curvedTheory}, to manipulate the pressure field in the domain.
Therefore, they observed opposite adaptations of the boundary layer growth compared to the ones we detected in a convex-curved \acrshort{tbl}. 
Their performed simulations showed an increased growth rate at the start of the curvature. 
Furthermore, the authors detected varying slopes for the growth rate depending on the pressure configuration.
These effects can be compared to the behaviour of the R100deg30 case of the curved-wall \acrshort{tbl} under study at the transition at the end of the curvature to the straight outlet where a similar effect can be observed.
This observation can be taken as an indication that the sensitivity of the boundary layer growth to the configuration may be driven more by the change of the curvature itself rather than the specific direction of the transition. 
...
It is evident that a significant disparity exists between the base and the optimised configuration.
In the straight inlet segment, both configurations demonstrate a constant value of $\beta$ at zero, which has been already described in detail in \autoref{sec:flatTBLVerif}.
...
In \autoref{fig:R100deg30Re1200p}, the wall pressure distribution in streamwise direction is depicted.
The streamwise direction is once more normalised with the reference boundary layer thickness. 
The pressure is visualised as a difference between the local pressure at the wall $p_w$ and the mean pressure at the wall within the curved segment $\langle p_c \rangle$.
In the straight segment both configurations exhibit a constantly decreasing pressure distribution with a similar gradient.
...
There is a substantial divergence in the behaviour of the two configurations at the start of the curved section.
In the base configuration, a drop of the pressure level is observed at the start of the curved wall. 
...
A similar behaviour in the opposite direction can be observed at the end of the curved segment.
A positive pressure jump can be detected in this area.
Afterwards, the pressure decreases with a constant gradient again.
Throughout the entire domain a pressure decrease of $\Delta(p_w-\langle p_c \rangle) \approx 0.09$ is visible in the base configuration.
The optimised configuration shows a different trend in the curved segment.
At the onset of the wall curvature, the pressure trend changes from decreasing to a constant level with a minor positive trend.
Minor fluctuations can be noticed throughout the entire curved segment.
However, the value remains constant on a \benennung{global} level at $\approx 0$ in the curvature. 
...
The local free-stream velocity $U$ relative to the free-stream velocity at the reference position $U_0$ is plotted over the normalised streamwise coordinate.
In both configurations, a constant positive trend is evident in the straight section.
As previously outlined, the two configurations diverge at the onset of the wall curvature.
...
At the end of the curved section, a converse behaviour is observed.
Overall, the base configuration shows a positive trend with an absolute difference of $\Delta U/U_0 \approx 0.08$.
In the optimised configuration, conversely, a relatively positive spike occurs immediately prior to the start of the curved segment.
Thereafter, a minor negative trend is evident, but with a lower absolute gradient compared to the base configuration.
In addition, local fluctuations are observable in the curved segment, similar as described in \autoref{fig:R100deg30Re1200p}.
As the curved section transitions into the straight outlet segment, a decrease in the velocity becomes evident, which recovers until the end of the domain is reached.
...
The positioning of a design point will modify the size of the cross-sectional area, which impacts the surrounding area as well.
...
This is due to the fact that the cross-sections located between the design points depend only on the spline shape selected by the optimiser which may lead to relatively high changes in the cross-sectional area through an unfavourable course of the spline.
...
In this context, we use the term global when analysing the general trend of a quantity over the entire curved section.
The local behaviour is defined as the short-range deviations of a quantity from this general trend.
Compared to the base configuration, a shift towards a more uniform pressure distribution on a global level is evident.
Subtracting the mean pressure results in an approximately zero pressure level in the curved segment. 
...
In the base configuration, the global pressure trend shows a decrease in the curved section, analogous to that in the straight inlet.
Since the pressure declines with a constant slope, this constitution results in a constant pressure gradient.
In conclusion, the capability of the optimisation algorithm to enforce a constant pressure differs significantly between the local and the global level when evaluated.
...
The findings indicate that the optimisation process leads to a substantial modification of the pressure field, as evidenced by the almost constant pressure level in the curved segment.
...
The decline in the pressure provokes a negative pressure-gradient, already introduced as \acrshort{fpg}.
...
To examine this in detail, an analysis of their positions in relation to the positions of the design points will be necessary, which is out of the scope of this thesis.
...
The following section compares the results of the cases R100deg90 and R500deg18.
First, the statistics extracted at $\mathrm{Re}_{\theta}=2000$ are analysed and compared with the flat \acrshort{tbl} simulation data from \citet{schlatter_assessment_2010}.
...
The plots comparing the base and optimised configurations within one case are added in the \autoref{chap:attachment}.
...
Up to the log-law region, the profiles match the reference profile, the linear law and the log law perfectly.
...
In the velocity profile, however, this effect  is only visible if the influence of the \acrshort{bc} extends significantly into the domain up to the region that is already plotted as the outer region in the boundary layer velocity profile.
...
The reduction is particularly pronounced for the case R100deg90. 
This effect is indicated by the geometry of this case.
As already shown by \citet{so_experiment_1973} and \citet{schwarz_convex_1996}, a convex curved geometry reduces the peak of the $\langle u'u' \rangle^+$-component of the Reynolds stresses. 
Since convex curvature has a stabilising effect on the \acrshort{tbl}, the turbulent fluctuations are suppressed  and consequently, the Reynolds stresses \citep{yang_assessment_2016}.
This effect strengthens with increasing intensity of the curvature; thus,  the reduction is more pronounced for the case R100deg90.
...
A reduction in the peak of the production term is observed, particularly for the case R100deg90. This behaviour was also reported  by \citet{gillis_turbulent_1983} and  is attributed to the strong convex curvature in this configuration.
...
Compared with R100deg90,  case R500deg18 exhibits  a larger \acrshort{tbl} growth rate. This difference is consistent with the stronger stabilising effect of the more intense convex curvature in the case R100deg90, which suppresses boundary-layer growth \citep{gillis_turbulent_1983,so_experiment_1973}.

The optimisation affects the \acrshort{tbl} growth differently in the cases R100deg90 and R500deg18.
For case R500deg18, $\delta_{99}$ is less affected by the curvature in the optimised configuration.
A similar behaviour was already detected for the case R100deg30.
A converse effect has been reported for a \acrshort{tbl} over a concave-curved wall by \citet{appelbaum_systematic_2025}.
In contrast, the optimisation further suppresses the growth of $\delta_{99}$ in the case R100deg90 when compared to the base configuration.
...
\end{subfigure}
\caption[Pressure analysis quantities: (a)friction coefficient; (b) Clauser parameter $\beta$; (c) pressure distribution, visualised as difference between local pressure and mean pressure in the curved section; (d) normalised free-stream velocity; all quantities plotted against normalised streamwise coordinate]{Pressure analysis quantities: (a)friction coefficient; (b) Clauser parameter $\beta$; (c) pressure distribution, visualised as difference between local pressure and mean pressure in the curved section; (d) normalised free-stream velocity; all quantities plotted against normalised streamwise coordinate, \lswatch[solid]{delta99thetacolone} base /\lswatch[solid]{delta99thetacoltwo} optimised R100deg90, and \lswatch[solid]{delta99thetacolthree} base / \lswatch[solid]{delta99thetacolfour} optimised R500deg18.},
...
For both cases, differences between the base and the optimised configurations are observed.
All configurations exhibit a peak near the inflow, resulting from the tripping of the boundary layer \citep{schlatter_turbulent_2012}.
...
In contrast, no comparable difference between the base and the optimised configuration is observed for the case R100deg90. 
Both configurations exhibit a similar deviation at the beginning of the curved segment and subsequently follow nearly identical trends until the end of the curved section.
...
Similar to the case R100deg30 discussed in \autoref{subsec:R100deg30}, both configurations  exhibit distinct peaks at the entrance and exit of the curved  segment. However, these peaks are less pronounced than those observed for the case R100deg30. 
...
For the case R100deg90, the peaks at the transitions into and out of the curved section are substantially larger than those observed for the case R500deg18.  
...
For the case R500deg18, the qualitative behaviour closely resembles that of the case R100deg30. 
...
In the case R500deg18, the optimised configuration results in an almost constant free-stream velocity of close to unity.
In contrast, the optimised configuration of the case R100deg90 still exhibits an increasing free-stream velocity, with an even steeper overall trend than the corresponding base configuration.
Additionally, pronounced local fluctuations are present, consistent with the behaviour observed in the distributions of $\beta$ and the wall pressure.
...
Although local fluctuations are still present in all analysed quantities, they are less pronounced than in the case R100deg30.
...
In contrast, the optimisation performs considerably less effectively for the case R100deg90.
...
One possible explanation is that the effective influence of each design point decreases with increasing curvature.
...
This effect is particularly pronounced for the case R100deg90.
Although the cases R100deg90 and R100deg30 share the same radius in the curved section, the longer curvature of the case R100deg90 results in a larger cumulative pressure-gradient effect.
